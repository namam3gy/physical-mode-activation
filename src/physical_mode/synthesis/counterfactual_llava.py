"""§4.6 cross-model — LLaVA-1.5 (CLIP-ViT-L/14) variant of pixel-space
gradient ascent.

Differences from `counterfactual.py` (Qwen2.5-VL):
  - LLaVA-1.5's processor produces pixel_values of standard shape
    `(1, 3, 336, 336)` rather than Qwen's patch-flattened
    `(T_patches, 1176)` layout. The optimization variable is the
    standard tensor; reconstruction is straightforward de-normalization
    + permute(C,H,W → H,W,C).
  - Visual-token mask resolution is the same generic recipe
    (model.config.image_token_id / image_token_index).

This module is a near-clone of counterfactual.py with only the
preprocessing-specific helpers replaced. The `gradient_ascent` and
`forward_get_layer_hidden` logic is reused from counterfactual.py via
re-import.
"""
...
from .counterfactual import (
    ...
    _resolve_image_token_id,  # noqa: F401  (re-exported for callers)
)


def pixel_values_from_pil_llava(
    pil_image: Image.Image,
    processor,
    prompt: str = "What will happen next?",
) -> torch.Tensor:
    """Return float32 pixel_values shape `(1, 3, H, W)` for LLaVA-1.5."""
...
def reconstruct_pil_llava(
    pixel_values: torch.Tensor,
    processor,
) -> Image.Image:
    """Inverse of standard CLIP normalization: `(1, 3, H, W)` → RGB PIL."""
...
def prepare_inputs_for_grad_llava(
    model,
    processor,
    pil_image: Image.Image,
    prompt: str = "What will happen next?",
) -> tuple[dict[str, Any], torch.Tensor]:
    """LLaVA-1.5 variant: pixel_values shape (1, 3, H, W)."""
    msgs = [{"role": "user", "content": [{"type": "image"}, {"type": "text", "text": prompt}]}]
    text = processor.apply_chat_template(msgs, tokenize=False, add_generation_prompt=True)
    raw = processor(images=[pil_image], text=[text], return_tensors="pt")
...
def gradient_ascent_llava(
    ...
    """Pixel-space gradient ascent for LLaVA-1.5.

    Mirrors `counterfactual.gradient_ascent` but uses LLaVA-1.5's
    standard `(1, 3, H, W)` pixel_values layout. eps clipping is per-
    pixel-channel (not per-patch as Qwen).
    """
    if isinstance(v_unit, np.ndarray):
        v_unit = torch.from_numpy(v_unit)
    v_unit = v_unit.to(device=model.device, dtype=torch.float32)

    inputs, pv_leaf = prepare_inputs_for_grad_llava(model, processor, pil_image, prompt)
