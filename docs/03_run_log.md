...
## 2026-04-24 VTI steering (M5 Phase 1-2, partial Sub-task 4)

- Phase 1 command: inline Python — `compute_steering_vectors` from
  `src/physical_mode/probing/steering.py`
...
- Outputs: `outputs/mvp_full_20260424-094103_8ae1fa3d/probing_steering/` +
  `steering_experiments/`
- Deep dive: `docs/08_m5_insights.md`

### Headline: causal evidence for a "physical object-ness" direction

**Phase 1 (direction)**: VTI vectors derived from forced-choice PMR labels.
Norms grow 5× through the LM (L5: 5.9 → L25: 31). Projection at L20
cleanly tracks cue axis (none 22.3 → both 42.7). Direction is real and
factorial-aligned.

**Phase 2 (intervention) — first-letter distribution on `line/blank/none`,
label `circle`, α=40**:
...
**Layer 10 α=40 flips 10/10 from "D: abstract" to "B: stays still"**. No
other layer budges at α=40. This is the first causal confirmation that a
single direction in the LM residual stream gates the "physical object"
vs "abstract shape" decision.

Sample pre/post:
- **baseline**: "D — This is an abstract shape and as such, it does not
  have physical properties that would allow it to fall, move, or change…"
- **L10 α=40**: "B) It stays still. — The circle in the image appears to
  be floating or suspended in space without any external force acting
  upon it. In such a scenario, the circle would remain stationary…"

The direction is "object-ness" (flips to B = physical-static), **not**
"gravity" (would flip to A = falls). Consistent with H7: physics regime
is label-driven, direction encodes the binary "abstract vs physical"
split.

### Caveats

- Forced-choice PMR scorer fires on option-listing ("cannot fall, move,
  or change direction") → PMR=1 is noisy here. First-letter flipping is
  the clean causal signal.
- Test subset = 10 stimuli only; replication on filled/blank/none etc.
  pending.
- α=40 is the only value that worked; finer sweep needed to map the
  threshold precisely.
...
|---|---|---|---|
| H-boomerang | 확장 (M4) | **확장 + 인과** | intervention 으로 causal 활성 |
| H-locus | 후보 (M4) | **지지 (early-mid L10)** | L10 이 causal sweet spot |
| H-regime (new) | — | **후보** | direction 은 object-ness 이지 which-physics 가 아님 |

### Unlocks / deferred

- Phase 3 (SIP activation patching) is scoped but not executed — would
  need `capture_lm_attentions=True` re-capture plus the patching
  machinery. On the ROADMAP §3 M5 detail.
- SAE on L10 residual stream is a natural next step (§4 additional
  idea; would decompose object-ness direction into finer features).

## 2026-04-24 LM probing (M4, Sub-task 3)
...
- Deep dive: `docs/07_m4_insights.md`
...
→ All captured LM layers discriminate PMR with AUC ~0.94-0.95. The peak at
layer 20 aligns with Neo et al. 2024's LLaVA claim that object features
crystallize in mid-to-late LM layers. **Information survives the LM almost
losslessly**; the ~29 pp gap to behavioral forced-choice accuracy (~0.66)
must come from the discrete generation step.
...
→ Physics > geometry from layer 5 onward — the "ball" label in the prompt
primes the LM *before* any residual updates. Final-layer amplification
(L20 → L25) boosts the physics margin from 0.9 to 4.0.
...
Object-induced margin: +0.6 at L25 (line → textured). **Label-induced
margin: +4.0 (flat bias across all stimuli)**. Label ≈ 7× object effect
inside the LM. Consistent with H7/H4.

**Switching layer** is trivially 5 for all 480 samples when using
max-logit-per-category — physics is already ahead at the earliest captured
layer because of the "ball" label. This metric is uninformative for
label-primed prompts; revisit with a label-free prompt variant.

### Hypothesis update

| H | prior | post-M4 | change |
|---|---|---|---|
| H-boomerang | 지지 (M3) | **확장** | 정보가 LM 전 구간 살아남음 — 게이팅은 decoding 에서 |
| H7 (label → regime) | 후보 (M2) | **지지** | Label prior 가 L5 에서 physics margin 을 전체 shift |
| H-locus (new) | — | **후보** | 병목은 LM final layers + decoding head. ST4 의 patching target. |

### Unlocks

- **Sub-task 4 (M5) target 이 좁혀짐**: LM layer 20-27 residual stream 과 decoding head
  가 개입 우선순위. `capture_lm_attentions=True` 로 재-capture 하면 SIP
  patching 즉시 실행 가능.
- **§4 아이디어 갱신**: 4.9 "label 없는 프롬프트" 는 switching-layer 메트릭을
  유효화하는 직접 테스트 — priority 상승.
