...
section: §4.11
date: 2026-04-26
status: complete (4-model M8d)
hypothesis: H7 categorical follow-up — labels select physics regime, not just PMR (binary)
---

# §4.11 — Categorical H7 follow-up: 4-model M8d regime distribution
...
Models: Qwen2.5-VL / LLaVA-1.5 / LLaVA-Next / Idefics2 (InternVL3 not
run on M8d).
...
1. **Qwen + Idefics2 are saturated kinetic across all cells**. Only
   visible exception: Qwen on `person × exotic` (`statue`) shows ~30%
   static — `statue` is exotic enough that even the saturated SigLIP
   architecture defers slightly. Otherwise: ~95% kinetic everywhere.
...
4. **Cross-model contrast on `person × abs`**: where the label is
   weakest (stick figure on a horizontal-event person stim):
   ...
   This 4-model gradient is the granular form of the M9 H7 finding:
...
2. **No InternVL3 row** because M8d wasn't run for InternVL3.
3. **n ≈ 40 per cell** is small enough that ±5 pp swings are noise.
   The ≥10 pp differences in the headlines are robust; the smaller
   ones are suggestive.
4. **Horizontal-event subset only** (per the M8d insight that this is
