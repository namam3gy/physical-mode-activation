...
5. **`references/project.md` is the research plan; `references/roadmap.md` is the living execution doc derived from it.** At every step, refer to both. Update `roadmap.md` (status table, hypothesis-scorecard one-liner, additional ideas) whenever a milestone completes or a finding contradicts a hypothesis. Treat `project.md` as read-only; major spec revisions go into the roadmap. **Two split-out docs** (extracted 2026-04-29 to keep roadmap.md compact): full hypothesis evidence chains live in `docs/hypotheses.md` (the roadmap §1.3 keeps only one-line statuses); the project changelog lives in `docs/CHANGELOG.md` (the roadmap §6 is just a pointer). When a hypothesis flips → update both `docs/hypotheses.md` and the roadmap §1.3 table; when a milestone completes → write the deep dive in `docs/insights/m{N}_*.md`, append a row to `docs/CHANGELOG.md`, and add (or update) the row in roadmap §3.
