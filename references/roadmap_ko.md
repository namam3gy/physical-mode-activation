...
| **H-encoder-saturation** (M6 r2 도출; M8c 정련; §4.5 인과 확인; M9 CI 강화; **M6 r4 비-CLIP-일반화**) | 행동 PMR(_nolabel) 포화는 (a) vision encoder 표상 포화 (M6 r2/r3/r4 probe AUC) 와 (b) 입력-맥락 단순성 (합성 미니멀 stim) 의 결합. 실 사진은 scene-context 잡음 추가로 포화된 인코더에서도 PMR 감소. **인코더 family (비-CLIP vs CLIP) 가 (a) 의 *인과* driver** — 4-모델 M8a 사슬: Qwen SigLIP / Idefics2 SigLIP-SO400M / InternVL3 InternViT 모두 AUC ≥ 0.89 + PMR ≥ 0.84; LLaVA CLIP-ViT-L 만 AUC 0.73 / PMR 0.18 으로 하락. **M9 부트스트랩 CI** (3 모델 × 3 stim): 비-CLIP CI [0.80, 0.92] vs CLIP [0.14, 0.37] 합성에서 — 완전 분리; 사진에서 3 모두 [0.18, 0.67] 로 수렴. | **인코더-family 수준에서 인과 확인 + 비-CLIP-일반화 + cross-stim 부트스트랩 검증 (M9 + M6 r3 + M6 r4)** | M6 r2b: Qwen vision AUC 0.99 / behavioral PMR(_nolabel) 0.95; LLaVA AUC 0.73 / behavioral 0.38; InternVL3 미캡처지만 behavioral PMR(_nolabel) 0.99 (포화 프로파일 일치). **M8a (2026-04-25)**: 도형 간 paired-delta — Qwen 5/5 0 근처/음수; LLaVA 5/5 ≥+0.125. **M8d (2026-04-25)**: 카테고리 간 paired-delta — Qwen +0.000 / +0.025 / +0.125; LLaVA +0.275 / -0.100 / +0.262. **M9 (2026-04-25)**: 3-모델 × 3-stim 부트스트랩 CI: 비-CLIP [0.800, 0.917] CLIP [0.140, 0.371] 분리; 사진 [0.183, 0.667] 수렴. **M6 r3 (2026-04-25)**: Idefics2 SigLIP-SO400M probe AUC 0.93 (4 레이어 평균). **M6 r4 (2026-04-25)**: InternVL3 InternViT probe AUC 0.89 / PMR 0.92 — 4-점 사슬이 포화를 비-CLIP-일반화. |
...
| **M6 r4** | **InternVL3 비전-인코더 프로브 (4-모델 사슬, 비-CLIP-일반화)** | M8a 추론 (1200+400 12 분) + 400 stim × InternViT 4 레이어 (47 초 캡처) + per-layer 로지스틱 회귀 프로브. **InternVL3 평균 AUC 0.89 / PMR(_nolabel) 0.92.** 4-점 사슬: Qwen 0.99/0.84, LLaVA 0.73/0.18, Idefics2 0.93/0.88, InternVL3 0.89/0.92. **3 개 별개의 비-CLIP 인코더 family (SigLIP, SigLIP-SO400M, InternViT) 모두 AUC ≥ 0.89; CLIP-ViT-L 만 0.73 으로 하락.** 논문 주장이 "SigLIP 포화"에서 "비-CLIP 인코더 포화; CLIP 비포화 (이 표본)"로 일반화. InternVL3 `vision_tower.encoder.layer` (단수) 속성 인식을 위한 vlm_runner 수정 필요. | ✅ | 2026-04-25 |
...
**헤드라인**: M8a 합성 stim 의 4-점 AUC ↔ 행동 PMR(_nolabel) 사슬:
...
**3 개 별개의 비-CLIP 인코더 family** (SigLIP, SigLIP-SO400M, InternViT) 모두 AUC ≥ 0.89 / PMR ≥ 0.84 도달. **CLIP-ViT-L 만 포화 미달** (0.73 / 0.18). 4 LM family 횡단 (Qwen2-7B, Mistral-7B, InternLM2-7B, Vicuna-7B), 인코더 family 가 통합 포화 driver.

**가설 업데이트**: H-encoder-saturation 이 "SigLIP 포화"에서 "비-CLIP 인코더 포화; CLIP 비포화 (이 표본)"으로 일반화. 논문급 헤드라인: 비전-인코더 family 가 합성-stim 천장 체제를 인과적으로 결정.

**구현 노트**: InternVL3 의 `vision_tower.encoder.layer` (복수가 아닌 단수) 인식 위해 `_resolve_vision_blocks` 수정 필요했음.

**주의**: Qwen + LLaVA AUC 는 M2 stim (M6 r2); Idefics2 + InternVL3 AUC 는 M8a. cross-stim mismatch; 인코더 probe AUC 는 합성 stim 에 거의 stim-invariant 이므로 footnote, 차단 아님.
