...
| M5b | ST4 Phase 3 — SIP + patching + SAE | Semantic Image Pairs + activation patching (visual-token full-state) on Qwen2.5-VL: 20 SIP 페어 (cue=both clean × cue=none corrupted) × 28 LM layer. **결과: L0-L9 패치 → 100% physics 회복 (IE=+1.0); L10-L11 → 60%; L12 → 30%; L14+ → 0%**. Sharp L10 boundary = decision-lock-in 레이어 (M5a 의 L10 single-causal-layer steering 결과와 매칭). H10 ("2-3 narrow IE bands") 정련: 하나의 contiguous L0-L9 범위. Head-level + attention knockout + SAE intervention 미해결. | partial ✅ (visual patching 완료; attention knockout / SAE 남음) | 2026-04-26 |
