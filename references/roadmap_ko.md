...
| **H-locus** (M4-derived; M5b SIP 로 2026-04-26 저녁 정련; **2026-04-26 밤 attention/MLP knockout 으로 추가 정련**) | Bottleneck 은 LM mid-layer (L10 specifically) 에 있으며 더 이전이나 decoding head 에는 없음. | **지지 + 두 번 정련 — L9 MLP 가 physics-mode commitment 를 construct 하고 L10 이 read out; attention 은 single-layer redundant** | M5a: L10 α=40 이 10/10 abstract → physical 응답 flip; 후반 layer 미동. M5a-ext Exp 3: L10 regime-flip 모든 cell 에서 성립. M5b SIP 패칭 (sufficiency, 2026-04-26 저녁): corrupted 의 L0-L9 hidden state 를 clean 의 값으로 패치하면 20/20 페어에서 physics 회복 (IE=+1.0); L10-L11 60%; L14+ 0. **M5b attention/MLP knockout (necessity, 2026-04-26 밤, 이번 라운드)**: **L9 MLP** output zero → 20/20 clean stim physics → abstract flip (IE_necessity=+1.0) — 프로젝트에서 가장 localized causal 발견. Attention knockout 모든 L0-L27 에서 0/20 break (single-layer attention 완전 redundant; residual stream + 살아남은 MLP 가 reconstitute). 부분 MLP necessity ring: L8 +0.4, L10 +0.6, L11 +0.4, L14 +0.4. Triangulation: L9 가 **sufficient (SIP) AND necessary (knockout)** — L9 의 MLP 가 physics-mode representation 을 *construct*, L10 이 *read*; off-by-one M5a/M5b reconciliation = 같은 decision boundary 의 두 view. H10 ("2-3 narrow IE bands") 정련 지지: 1 dominant L9 MLP band + 4 partial echoes; attention 은 IE band 없음. |
