...
| **H-locus** (M4-derived; M5b SIP 로 2026-04-26 저녁 정련; layer-level knockout 로 2026-04-26 밤 정련; **2026-04-27 per-head knockout 로 추가 확인**) | Bottleneck 은 LM mid-layer (L10 specifically) 에 있으며 더 이전이나 decoding head 에는 없음. | **지지 + 세 번 정련 — L9 MLP 가 physics-mode commitment 를 construct, L10 이 read out; attention 은 *layer 와 head resolution 모두에서* redundant** | M5a: L10 α=40 이 10/10 abstract → physical 응답 flip; 후반 layer 미동. M5a-ext Exp 3: L10 regime-flip 모든 cell 에서 성립. M5b SIP 패칭 (sufficiency): corrupted 의 L0-L9 hidden state 를 clean 의 값으로 패치하면 20/20 페어에서 physics 회복 (IE=+1.0); L10-L11 60%; L14+ 0. M5b layer-level attention/MLP knockout (necessity): L9 MLP zero → 20/20 flip (IE=+1.0); attention knockout 모든 L0-L27 에서 0/20 break. **M5b per-head attention knockout (2026-04-27, 이번 라운드)**: 20 stim × 7 layer (L8-L14) × 28 head = **196 (L,h) ablation 모두 IE = 0** — 모든 single head 가 dispensable. 부분 MLP necessity ring (L8 +0.4, L10 +0.6, L11 +0.4, L14 +0.4) 유지. Triangulation: L9 가 **sufficient (SIP) AND necessary (knockout)** — L9 의 MLP 가 physics-mode representation 을 *construct*, L10 이 redundant attention 으로 *read* (specific head 무관); off-by-one M5a/M5b reconciliation = 같은 decision boundary 의 두 view. H10 ("2-3 narrow IE bands") 완전 resolved: 1 dominant L9 MLP band + 4 partial echoes; attention 은 *어떤* resolution 에서도 narrow IE band *zero*. Mechanism 은 construction-and-broadcast, pull-through-a-specific-head 가 아님. |
...
| **H-direction-specificity** (§4.6-derived; 2026-04-26/04-27 수정; **5-모델 n=10 sweep 2026-04-28**) | v_L 방향 픽셀-공간 gradient ascent 가 PMR 을 뒤집고 매칭 magnitude random 방향은 못함 — regime flip 은 magnitude 가 아니라 방향 특이성 필요. | **supported across 5 architectures × 5 layers at n=10** | **5-모델 2026-04-28 layer sweep at ε=0.1**: random direction phys rate 모든 깨끗한 shortcut layer 에서 ≤ 0/50 (Qwen L10 random 1/10 = 10 % 가 lone exception, vs v_unit 100 % — separability 유지). Qwen 모든 5 layer ≥ 80 %, LLaVA-Next L20+L25 100 %, LLaVA-1.5 L25 40 %; Idefics2 + InternVL3 도 v_L projection 일관되게 상승 (gradient ascent 작동) — 즉 방향 특이성이 *projection 수준* 에서는 보존됨, PMR 로 translate 안되는 곳에서도. |
| **H-shortcut** (§4.6-derived; 2026-04-26/04-27 수정; **5-모델 n=10 sweep 2026-04-28**) | Shortcut 해석은 이미지 자체에 인코드 가능 — 픽셀 기반. | **supported, architecture-conditional with model-specific shortcut profile; encoder saturation 은 CLIP/SigLIP+Qwen 부분집합 안에서 breadth 와 상관이지만 엄밀히 인과적이지 않음 (Idefics2 falsifies)** | **§4.6 5-모델 n=10 sweep (2026-04-28)**: 각 architecture 가 자신의 shortcut LM layer profile 보유. **Qwen2.5-VL** (SigLIP+Qwen2-7B, AUC 0.99): L5/10/15/20/25 모두 broad shortcut (Wilson lower bound 0.49–0.72). **LLaVA-Next** (CLIP+AnyRes+Mistral-7B, AUC 0.81): L20+L25 (각 10/10). **LLaVA-1.5** (CLIP+Vicuna-7B, AUC 0.73): L25 만 (4/10, n=5 morning 보다 약함). **Idefics2** (SigLIP-SO400M+Mistral-7B, AUC 0.93): **테스트 어떤 layer 에도 깨끗한 shortcut 0** (v_L projection 깨끗하게 +38 상승함에도). encoder saturation 단독으로 픽셀-인코드 가능 breadth 야기 안 함; LM family / projector 설계도 매터. **InternVL3** (InternViT+InternLM3, AUC 0.89): protocol-saturated (baseline_pmr=1.0). Random control 은 모든 깨끗한 shortcut layer 에서 0/50 (Qwen L10 1/10 만 예외, v_unit 10/10 보다 훨씬 낮음). 픽셀-인코드 가능성은 여전히 *encoder→LM pipeline 의 경험적 속성*, M9 PMR-천장 + §4.7 결정-안정성 합류한 세 번째 architecture-level 시그니처 — 그러나 strict "capacity scales with saturation" 주장은 "법칙" → "CLIP/SigLIP+Qwen 부분집합 안의 패턴" 으로 격하. Insight: `docs/insights/sec4_6_cross_model_revised.md` (+ ko). |
...
| 2026-04-25 | **2026-04-25 burst (압축, 19 commit)** — 단일 일자에 multi-axis 증거 기반을 일괄 산출: **(a) 가설 개정** — M5a-ext Exp 3 가 `v_L10` 을 regime axis 로 reframe (+α kinetic / −α static / baseline D below threshold; H-direction-bidirectional 개정, H-regime 을 H7 qualifier 로 축소); M4b H2 null test 가 language prior 비대칭 노출 (circle override, ball enhancement 아님); M4c FC label-free 가 label confound 없이 M4b 재현; M6 r1 (LLaVA-1.5) 이 H2 를 **visual-saturation 가설** 로 재개정 (M4b 의 "circle suppression only" 는 Qwen 특이적). **(b) 신규 H-encoder-saturation 가설** 이 M6 r2 (LLaVA-1.5 vision AUC 0.73 vs Qwen 0.99) 통해 도입 + §4.5 Idefics2 swap 으로 **encoder-family 수준 인과 확인** (SigLIP-SO400M + Mistral-7B 가 Qwen 과 동일 패턴). **(c) 외부 타당성 push** — M8a (5 합성 도형), M8d (3 비-공 카테고리: car/person/bird), M8c (60 실사진), M8e (cross-source 통합): 포화 encoder → ceiling → H1/H7 headroom 없음; unsaturated → 측정 가능. 사진이 Qwen PMR(_nolabel) 을 18-48 pp 낮춤 (합성-stim 단순성이 공동 인자). H7 을 **cross-category** 로 승격 (LLaVA M8d 3/3 ✓). **(d) 5-모델 점 (3 비-CLIP + 2 CLIP) 에서 AUC↔PMR 사슬 lock** — M6 r3 (Idefics2 vision AUC 0.93), M6 r4 (InternVL3 InternViT AUC 0.89), apples-to-apples 4-모델 M8a-stim 재캡처, M6 r5 (M8c 사진 cross-stim), M6 r6 (LLaVA-Next-Mistral 2번째 CLIP 점, PMR 0.70 [0.65, 0.74] + cross-stim 부록). H-LM-modulation 은 suggestive-only (Idefics2 M8d H7 CI 0 닿음). **(e) M9 일반화 audit** (논문 Table 1 후보): 9 (모델 × stim) 셀 × 5000 부트스트랩; 비-CLIP CI [0.80, 0.92] vs CLIP [0.14, 0.37] (합성에서 완전 분리); 사진 [0.18, 0.67] 로 압축. **(f) Roadmap 우선순위 재배치**: M8a/c/d + §4.5/4.6/4.10 promotion; M5b + M6 r3+ optional 로 강등. Insights (모두 영문): `m4b_label_free.md`, `m4c_fc_label_free.md`, `m5a_ext_bidirection_and_label.md`, `m6_cross_model_llava.md`, `m6_r2_cross_model.md`, `m6_r3_idefics2_probe.md`, `m6_r4_internvl3_probe.md`, `m6_r5_m8c_photo_probe.md`, `m6_r6_llava_next.md`, `m8a_non_circle_shapes.md`, `m8c_real_photos.md`, `m8d_non_ball_categories.md`, `m8e_cross_source.md`, `m9_generalization_audit.md`, `encoder_swap_idefics2.md`. | `f8f0fdd`..`524e32b` (19 commit) |
...
| 2026-04-26 | **M6 r7 (M2 cross-model) 완료**: M2 protocol (5-axis factorial, T=0.7, 480 stim × 3 라벨) 을 LLaVA-Next + Idefics2 + InternVL3 에 LM 활성화 캡처와 함께 확장 (Qwen + LLaVA-1.5 기존). 순차 GPU 1 chain: 5 capture run + 5 label-free run ~22분. **5-model PMR(_nolabel) 사다리 (M2 stim 위 apples-to-apples)**: LLaVA-1.5 0.383 / LLaVA-Next 0.790 / Qwen 0.938 / Idefics2 0.967 / InternVL3 0.988. H1 ramp 는 LLaVA-1.5 만 깨끗 (+0.30 range); 나머지는 천장. **H2 paired-delta 가 3가지 architecture-conditional 패턴** — LLaVA-1.5/LLaVA-Next 모두 양수 (classical H2), Qwen/Idefics2 비대칭 (circle/planet < 0; "circle override"), InternVL3 ≈ 0 (saturated). Per-model v_L10 추출; LLaVA-1.5 만 class-balanced (n_neg=105); LLaVA-Next/Idefics2/InternVL3 는 n_neg=9/5/1 — M2 만으로는 v_L10 추출 불가. Insight: `docs/insights/m2_cross_model.md` (+ ko). | `4bd4623` |
| 2026-04-26 | **§4.6 cross-model 완료 (null — 픽셀-인코드 가능성은 encoder-saturation 특이적)**: (a) Transfer test: Qwen-derived synth stim → 4 비-Qwen 모델. **0/140 flip**. LLaVA-1.5/LLaVA-Next/Idefics2 baseline=synth=0; **InternVL3 음의 transfer** (큰 ε 에서 baseline 1.0 → unconstrained 0.2). (b) LLaVA-1.5 per-model gradient ascent: M2 만에서 class-balanced v_L10 가진 유일한 모델. GPU 1 에서 35 run / 9.8분. **모든 ε 에서 0/5 v_L10 flip**, 0/15 random. **결정적 관찰**: gradient ascent 가 projection 수준에서 성공 (v_L10 8 → 150-200, Qwen 의 43-180 와 비슷) 그러나 PMR flip 안 함 — *projection 수준* 과 *행동 수준* 의 분리. **H-shortcut 과 H-direction-specificity 수정 → Qwen-scoped**. 픽셀-인코드 가능성은 encoder-saturation 특이적, M9 PMR-천장 / §4.7 결정-안정성 천장과 평행. 신규 모듈 `src/physical_mode/synthesis/counterfactual_llava.py` (표준 CLIP 변형). Insight: `docs/insights/sec4_6_cross_model.md` (+ ko). | `ec2aa77` |
| 2026-04-26 (저녁) | **H9 + M5b SIP+patching 완료**: (a) **H9 cross-model layer-wise probe AUC** — 5 모델 × 5 captured layer (5/10/15/20/25); Qwen 이 L5 부터 AUC 0.97 plateau, LLaVA-1.5 가 0.77 plateau, 0.85 도달 안 함; LLaVA-Next 0.73→0.79; Idefics2 / InternVL3 saturated (n_neg = 5, 1 — fullfit fallback). H9 reframe: "earlier-vs-later" 가 아니라 "saturated 인코더는 L5 이전에 plateau; unsaturated 는 saturated plateau 도달 안 함." Insight: `docs/insights/h9_layer_switching.md` (+ ko). (b) **M5b SIP + activation patching on Qwen2.5-VL** — 20 SIP 페어 (cue=both clean × cue=none corrupted, M2 stim) × 28 LM layer × H200 ~8분. **Sharp L10 boundary**: L0-L9 패치 → 20/20 physics 회복 (IE=+1.0); L10-L11 → 12/20 (0.6); L12 → 6/20 (0.3); L13 → 2/20 (0.1); L14+ → 0/20. M5a (L10 single-causal-layer steering) 를 L10 = decision-lock-in *경계* 로 정련, 정보는 L0-L9 모두 존재. H10 ("2-3 narrow IE bands") 부분 지지 with revision: 하나의 contiguous L0-L9 범위. H-locus 정련. 신규 모듈 `scripts/m5b_sip_activation_patching.py` (forward-hook visual-token replacement at prefill). Insight: `docs/insights/m5b_sip_activation_patching.md` (+ ko). | (이 commit) |
| 2026-04-26 (저녁) | **§4.6 cross-model 재수정 — 픽셀-인코드 가능성은 Qwen 한정 아님 (overnight followup)**: 오전의 Qwen-scoped finding 이 **wrong-layer-choice artifact** 였음을 발견. M2 captures 는 saturated 모델에 n_neg = 1-9; 신규 M8a captures (3 모델 × ~50분 GPU 1) 가 n_neg = 100-280, much cleaner. M8a vs M2 v_L cosine 분석: Idefics2 ~0.79 (동일 방향; class imbalance robust), InternVL3 ~0.7+, LLaVA-Next ~0.4 (moderate). Class imbalance 가 *문제 아니었음*. **수정은 layer 선택**: LLaVA-1.5 §4.6 layer sweep (L5/L15/L20/L25; L10 이 이전 null) — **L25 가 ε=0.2 에서 5/5 v_L flip, ε=0.1 에서 4/5**; 모든 layer 의 random control 0/15. LLaVA-1.5 는 32 LM layer (L25 = 78% 깊이) vs Qwen 28 (L10 = 36%) — different relative depth. LLaVA-1.5 L25 ε=0.2 응답 샘플: "The circle will be hit by a ball." (vs baseline "filled in with color"). Insight: `docs/insights/sec4_6_cross_model_revised.md` (+ ko). 이전 `sec4_6_cross_model.md` doc 에 REVISION NOTICE 헤더 추가됨. **Open**: Qwen layer sweep (Qwen 도 L25 에 second shortcut layer?), LLaVA-Next/Idefics2/InternVL3 의 per-model gradient ascent (custom counterfactual_<model>.py 필요). | `165a525` |
| 2026-04-26 (밤) | **M5b cross-model SIP + attention/MLP knockout 완료**: (a) **LLaVA-1.5 cross-model SIP+patching** (n=15 × 32 layer, GPU 1 에서 ~12분) — IE-curve 모양이 Qwen 의 sharp-then-declining 프로파일을 재현, 그러나 **decision-lock-in 이 L20 (62.5% relative depth) 에서 시작 vs Qwen L10 (36%)**. Idefics2 / InternVL3 은 M2 open-prompt 에서 SIP 구축 너무 saturated (n_neg = 0, 0). H-locus 재프레임: 각 VLM 이 model-specific relative depth 의 자체 decision-lock-in layer 보유; locus 는 cross-model 존재하지만 다른 위치. Insight: `docs/insights/m5b_sip_cross_model.md` (+ ko). (b) **Attention + MLP knockout (necessity test)** on Qwen2.5-VL: 20 clean SIP stim × 28 LM layer × 2 ablation × GPU 1 ~18분. **Attention knockout: 모든 layer L0-L27 에서 IE = 0** — single-layer attention 완전 redundant; residual stream + 살아남은 MLP 가 reconstitute. **MLP knockout: L9 IE = +1.0 (유일 necessary, 0/20 retain physics)**, partial necessity ring L8 +0.4 / L10 +0.6 / L11 +0.4 / L14 +0.4. M5b SIP (sufficiency) + M5a (steering) 와의 triangulation: **L9 MLP 가 residual stream 에 physics-mode commitment 를 *construct*; L10 이 attention 으로 *read*.** Off-by-one M5a/M5b reconciliation = 같은 decision boundary 의 두 view — 프로젝트의 가장 localized causal 발견. H-locus 두 번 정련; H10 ("2-3 narrow IE bands") 정련 지지 with one dominant L9 MLP band + 4 partial echoes. 신규 모듈 `scripts/m5b_attention_mlp_knockout.py` (prefill-only forward hook 으로 self_attn 또는 mlp output zero). Insight: `docs/insights/m5b_attention_mlp_knockout.md` (+ ko). | `a96029b` |
| 2026-04-27 | **M5b per-head attention knockout 완료 (null result, 의미 있는 종류)**: 20 clean SIP stim × 7 LM layer (L8-L14) × 28 attention head = **196 (L,h) ablation 모두 IE = 0** — 모든 single head 가 dispensable; GPU 1 에서 ~36분. Hook 이 prefill 시 o_proj input 의 slice `x[..., h*head_dim:(h+1)*head_dim]` 을 zero (clean per-Q-head ablation; GQA-safe). L8-L14 부분-MLP-necessity zone 에서 "single decision-carrying head" 가설 배제. **Mechanism 결정**: L9 MLP 가 residual stream 에 physics-mode commitment 를 *construct*; L10 이 진짜로 diffuse 한 attention 으로 *read* — specific head 무관. H10 ("2-3 narrow IE bands") 완전 resolved: 1 dominant L9 MLP band + 4 partial echoes; attention 은 *어떤* resolution 에서도 narrow IE band *zero*. H-locus 세 번 정련. 한계: single-head 만 — multi-head combination ablation (L9 의 top-N visual-attention head 동시) 이 cumulative necessity 보일 수 있음. 신규 모듈 `scripts/m5b_per_head_attention_knockout.py` (o_proj 위 forward_pre_hook). Insight: `docs/insights/m5b_per_head_attention_knockout.md` (+ ko). | `83a0995` |
| 2026-04-27 | **M5b SAE intervention on Qwen vision encoder 완료 (positive 결과, advisor-checked)**: `vision_hidden_31` (마지막 SigLIP layer, pre-projection, 1280 dim) 의 622K visual token 위 5120-feature SAE 학습 (4× 확장, tied weight, input z-score, λ=1.0, 5K step, H200 에서 1.1분). 최종 recon = 0.023, 토큰당 7.3% active. Mvp_full 예측 (310 phys / 19 abs stim) 으로 `delta = mean(z | physics) − mean(z | abstract)` ranking. **인과 intervention** (n=20 clean SIP stim × `model.visual.blocks[-1]` 위 hook 이 target feature 의 raw-scale 기여만 빼는 — Bricken et al. trick, lossy decode round-trip 회피): top_k=1 → 1.0, top_k=5 → 0.6, top_k=10 → 1.0 (non-monotone, 미해결), **top_k=20 → 0.0 (full break, 20/20 abstract; mass=49.23)**. **초기 bottom-of-ranking random control 은 magnitude unmatched (mass ≈ top-20 의 1% — L1 penalty 가 inactive feature 죽임); advisor 가 flag. 수정: mass-matched random k=20 (mass=40.97 = top-20 의 83%) → 1.0 (20/20 retain physics)** — magnitude-driven encoder collapse 가 아닌 direction-specificity 확인. Single mass-matched set 획득 (heavy-tailed mass 분포가 3-set replicate 방해; binary outcome unambiguous). Caveat: feature 3313 (mass=14, ~3× next) 은 high-baseline outlier — likely "general image content"; future ranking by Cohen's d 가 filter. Triangulation 완성: input → encoder ~20 physics-cue feature → L0-L9 → L9 MLP commitment → L10 read-out → letter. 신규 모듈 `src/physical_mode/sae/{train,feature_id}.py`; driver `scripts/sae_train.py`, `scripts/sae_intervention.py`. Insight: `docs/insights/m5b_sae_intervention.md` (+ ko). M5b 완전 완료 (5개 하위 작업 모두). | (이 commit) |
| 2026-04-28 | **Overnight chain B → A → C 완료 (5-모델 §4.6 n=10 + Qwen 32B M2)**: 17:40 KST 2026-04-27 launch, 21:42 KST 같은 날 종료 (~4 시간 wall). (B) Qwen + LLaVA-1.5 + LLaVA-Next §4.6 를 n=10 stim/layer 로 재실행 (`line_blank_none_fall_*` 가 10 까지만 있어 n=50 불가능). n=10 이 Wilson CI 를 n=3 [0.44, 1.00] → [0.69, 1.00] 으로 좁힘. **n=10 이 n=3-5 의 100 % 주장 over-confidence 노출**: LLaVA-1.5 L25 4/5 → 4/10 (Wilson [0.17, 0.69]); Qwen L15/L20 3/3 → 8/10. (A) Idefics2 + InternVL3 모듈 추가 (`counterfactual_idefics2.py` 5-tile + pixel_attention_mask, `counterfactual_internvl3.py` single 448×448) + driver + 실행. **Idefics2 anomaly**: L5/10/15/20/25 모두에서 깨끗한 shortcut 0, v_L projection 모든 run +38 깨끗하게 상승함에도 — "encoder saturation 단독으로 shortcut breadth 야기" falsify. 두 후보 설명 (deeper-layer L26-31 미테스트 vs perceiver-resampler 병목) 미해결. **InternVL3 protocol-saturated**: `line_blank_none_fall_*` baseline_pmr=1.0; §4.6 "circle" 프롬프트 + InternVL3 의 saturated InternViT pipeline 이 M2 에서 flip 할 abstract-baseline stim 안 남김. (C) Qwen 32B M2 (open, 1440 stim, 16 분 wall): aggregate PMR 0.926 ≈ 7B 0.931 — 5× 스케일링 도움 안됨. 신규 §4.8 insight `docs/insights/sec4_8_pmr_scaling.md` (+ ko); 주요 shift `abstract_reject` 0.002 → 0.065, label gap 절반 (H2 약화 그러나 dissolve 안됨). H-shortcut 이 "capacity scales with saturation" → "CLIP/SigLIP+Qwen 부분집합 안의 model-specific shortcut profile; Idefics2 anomaly" 로 격하. 5-모델 summary script + figure 재생성; 5-패널 layout `docs/figures/sec4_6_cross_model_layer_sweep.png`. 신규 chain script `scripts/overnight_b_a_c.sh`. | (이 commit) |
| 2026-04-27 (오후) | **§4.6 cross-model 3-architecture layer sweep 완료 (Qwen + LLaVA-Next 추가; LLaVA-1.5 는 overnight 에서 이미 완료)**: `sec4_6_cross_model_revised.md` 의 두 open 항목 모두 해결. (a) **Qwen layer sweep** (`scripts/sec4_6_qwen_layer_sweep_unified.py`, 단일-프로세스): L5/L15/L20 추가 (기존 L10 morning + L25 smoke 와 결합). GPU 1 에서 ~10분에 18 runs. **결과**: L10/15/20/25 모두 100 % v_unit flip (각 3/3, L10/L25 는 5/5); L5 marginal 33 %; random 모든 layer 0. Qwen 은 4-5 shortcut LM layer 보유, 1 개 아님. (b) **LLaVA-Next per-model gradient ascent**: 신규 모듈 `src/physical_mode/synthesis/counterfactual_llava_next.py` 가 AnyRes 5-tile pixel_values 구조 처리 (per-element eps clip, base-tile 재구성). Driver `scripts/sec4_6_llava_next_layer_sweep_unified.py` 가 30 runs (5 layer × 3 stim × 2 config) 을 ~25분에. **결과**: L20+L25 둘 다 3/3 (100 %); L15 borderline (1/3 v_unit 가 1/3 random 과 동률 — noise threshold); L5/L10 null. LLaVA-Next 는 2 shortcut layer 보유. (c) **Cross-model summary** (`scripts/sec4_6_cross_model_layer_summary.py`): Qwen + LLaVA-1.5 + LLaVA-Next 를 통합 테이블 + 3-패널 figure (Wilson CI per layer) 로 집계. Architecture-level 스케일링 lock: ***shortcut LM layer 의 개수* 가 encoder saturation 에 따라감** — LLaVA-1.5 (CLIP, AUC 0.73, 1 layer) < LLaVA-Next (CLIP+AnyRes, AUC 0.81, 2 layer) < Qwen (SigLIP, AUC 0.99, 4-5 layer). H-shortcut 이 "model-conditional layer" 에서 "architecture-conditional with capacity scaling on encoder saturation" 로 promote; 픽셀-인코드 가능성은 이제 encoder saturation 의 **세 번째** architecture-level 시그니처 (M9 PMR 천장 + §4.7 결정-안정성 천장에 합류). `sec4_6_cross_model_revised.md` 의 한계 1+2 해결; Idefics2 + InternVL3 per-model 모듈은 여전히 open (AUC 0.93 / 0.89 기반으로 Qwen 처럼 동작 예측). 신규 figure `docs/figures/sec4_6_cross_model_layer_sweep.png`. Insight: `docs/insights/sec4_6_cross_model_revised.md` (+ ko). | (이 commit) |
| 2026-04-27 (저녁) | **M5b SAE intervention 수정 (Cohen's d + multi-seed random + dense k-sweep + cluster pivot)**: 오전 run 의 advisor-flagged 약점 3개 처리. (a) **Cohen's d 랭킹**: `feature_id.py` 확장하여 pooled-std-normalized delta 계산; 신규 `scripts/sae_rerank_features.py` 가 `feature_ranking.csv` 재생성 (`mean_phys/mean_abs/std_phys/std_abs/pooled_std/delta/cohens_d`). 5120 feature 전체 Spearman ρ = 0.98 그러나 delta-top-50 안에서는 ρ = 0.47 — high-delta region 의 랭킹 불안정; **top-20 7/20 turnover**. Feature 3313 이 rank 3 (delta) → ~rank 50 (Cohen's d) — high-baseline-noise outlier filter 가 의도대로 동작. (b) **Multi-seed mass-matched random**: `sae_intervention.py` 확장하여 `--rank-by {delta,cohens_d}` + multi-seed loop (seed 42..91, main window [0.7×, 2.0×] 부족 시 fallback [0.5×, 2.5×]). Top-30 에 대해 seed 42 안에서만 3 set: mass 23.4 / 24.7 / 33.4 (top-30 mass=32.7 의 72/76/102 %) → 3 set 모두 20/20 physics-mode (Wilson CI [0.84, 1.00]). (c) **Dense k-sweep (Cohen's d, k ∈ {1,2,3,5,7,10,15,20,30})**: top-30 → 0/20 (Wilson CI [0.00, 0.16]); top-20 → 2/20; top-15 → 20/20 (완전 회복); top-5/7/10 → 12/20. (d) **Stim-cluster pivot 으로 비단조성 해결**: 8 line_blank stim 모두 k=5/7/10 에서 함께 flip + k=15 에서 함께 회복; 12 non-line_blank stim 은 mid-range 에서 절대 flip 안 함. Mid-range "0.6" rate 는 *결정적이고 stim-cluster-conditional*, aggregate-rate 노이즈 아님 — rank 11-15 feature 가 line_blank-specific 손상을 보상하는 "abstract suppressor" 임을 시사. Headline 이 "single random control 1.0 (Wilson CI [0.84, 1.00])" 에서 "3 random control × 20 stim = 60 trial 모두 1.0 (aggregate Wilson CI [0.94, 1.00])" 로 tighten; lower-bound gap from 1.0 가 0.16 → 0.06 으로 ~2.7× 좁아짐. Cohen's d 가 이제 canonical ranking; raw delta 는 back-compat 용으로 유지. 신규 스크립트 `scripts/m5b_sae_figures.py` (Wilson CI + cluster pivot 의 수정된 figure 가 `docs/figures/m5b_sae_intervention_revised.png`). Insight (영어 + 한글) 업데이트. 한계 1/2/3 해결. | (이 commit) |
| 2026-04-28 (planning) | **Track B 결정 (ICLR 2027 / NeurIPS 2027 grade submission)** — 신규 plan 문서 `references/submission_plan.md` (20주 일정, 3 pillar: Multi-prompt / Controlled architectural counterfactuals / Marr 3-level reframing) + `references/paper_gaps.md` (4 gap G1-G4 → fix-track 매핑). §1.4 venue 갱신; §3.X Track B 우선순위 표 추가; M-MP / M-PSwap / M-LMSwap / M-Marr 마일스톤 row 추가. 5-signature framing 결정: **appendix-relegation > pruning** — 5 측정 모두 보존하되 Marr level (Computational PMR / Representational M3+M4 / Mechanistic M5a+M5b) 로 재배치. Multi-prompt 메뉴에서 counting + Michotte + identity prompt drop. World-model framing (production VLM = implicit world model; 첫 단계 = "어떤 종류의 세계인가?") 채택. EMNLP 2026 deferred. | (이 commit) |
| 2026-04-28 (M-MP Phase 1+2 완료) | **5-모델 multi-prompt cross-task generalization Phase 1 + Phase 2 ✅** — Phase 1 stratified smoke (n=48 stim × 3 prompt × 3 label × 5 모델 = 2160 추론, ~15분). Phase 2 full (n=480 × 3 prompt × 3 label × 5 모델 = 21,600 추론, GPU 1 chain ~3시간). **헤드라인**: H2 paired-delta 가 **5 모델 × 3 prompt = 15 cell 모두 양수** (range +0.006 to +0.344). Saturation × prompt 상호작용: saturated (Qwen/Idefics2/InternVL3) → describe 가 가장 informative; unsaturated CLIP cluster (LLaVA-1.5/LLaVA-Next) → open or yesno. 0/2160 unparseable on `meta_phys_yesno`. Phase 1↔2 ±0.018 PMR (stratified subset 무편향). 신규: 2 prompt variant in `prompts.py`, 5 multi_prompt_*.py config, score_meta_yesno + score_describe + score_for_variant in pmr.py, `m_mp_summarize.py` 분석 파이프라인, `m_mp_describe_label_helper.py` Claude-rater hand-label gate (5/5 PASS, agreement 0.88-1.00 / κ 0.74-1.00). Phase 3 prep: `06_vti_steering.py` + `sae_intervention.py` 에 `--prompt-variant` / `--prompt-mode {fc, open, describe_scene, meta_phys_yesno}` 플래그. Insight: `docs/experiments/m_mp_phase1.md` + `m_mp_phase2.md`. | (이 commit) |
| 2026-04-28 (M-MP Phase 3 + post-projection SAE 완료) | **M-MP Phase 3 cross-prompt M5a + M5b (Qwen + Idefics2 × 2 신규 prompt) ✅** + **M5b post-projection SAE on Qwen ✅** — Phase 3 결과가 G1 fix 를 "task-agnostic physics-mode commitment" → **"Qwen-specific generative vs categorical mechanism dissociation"** + **architecture-conditional breadth** 로 정련: (a) Qwen M5a + M5b 모두 `describe_scene` 에서 flip/break (10/10 / 0/10), `meta_phys_yesno` 에서는 NOT (0/10 / NO break) — **Qwen 내부에서**, physics-mode mechanism 은 generative-prompt-conserved + categorical-blocked. (b) Idefics2 M5a + M5b 모두 `describe_scene` flip 안 함 (0/10 / NO break); `shaded/ground/both` ball 에서만 ablation 이 kinetic→suspended framing shift 산출 — single-cell finding, architecture-level 아님. **Audit-revised (2026-04-28)**: cross-method-agreement 가 **1 positive/positive cell (Qwen × describe) + 3 null/null cell** — defensible 그러나 n=1 positive coincidence + 3 shared null. Idefics2 NULL 이 sanity check 로 "kinetic-verb-production specifically" + framing-shift 정량화로 정련. 신규 `score_describe` lexicon: "impact", "about to (fall|hit|impact|land|drop|bounce)", "in motion". Insight: `docs/experiments/m_mp_phase3.md`. **Post-projection SAE on Qwen**: `model.model.visual.merger` (3584-dim LM-space) 위 14336-feature SAE, 480 × 324 = 155K token, 6분 학습. Top-k Cohen-d ablation: k=20 → 0/20 break; k=5 → intact; random ×3 mass-matched 모두 1.0. Pre-projection (5120-feature SAE on `vision_hidden_31`) 과 동일 k=20 임계 — merger 가 physics-mode commitment localization 보존. 신규: `scripts/m5b_capture_post_projection.py`, `sae_intervention.py --hook-target {block, merger}`. Insight: `docs/insights/m5b_post_projection_sae_qwen.md`. | (이 commit) |
| 2026-04-28 (저녁 M5b cross-model + §4.6 deeper-layer) | **M5b SAE intervention cross-model (round 2) + §4.6 Idefics2 L26-31 deeper-layer disambiguation** — (a) **Cross-model SAE round 2**: round 1 이 `vision_hidden_23` 을 균일 사용했으나 LLaVA `vision_feature_layer=-2` + Idefics2 `last_hidden_state` 가 다른 layer 소비함을 발견 → 모델별 actually-consumed layer 에서 재학습 (LLaVA-1.5 vis22, LLaVA-Next vis22, Idefics2 vis26, InternVL3 vis23 기존). 신규 플래그 `sae_intervention.py --prompt-mode open --vision-block-idx --test-subset --stimulus-dir`; `sae_train.py --pmr-abs-threshold` (saturated baseline 모델용); `feature_id.py` 청크화 (5-tile Idefics2 2.5M token OOM 방지). **결과**: Qwen k=20 break (5120 feature 의 0.4 %); Idefics2 k=160 break (3.5 %); InternVL3 k=160 break (3.9 %); **LLaVA-1.5 + LLaVA-Next NULL at any k ≤ 160** (CLIP encoder cluster). 3 mass-matched random control × 5 모델 모두 1.0. Effect concentration 이 M3 vision probe AUC ladder (Qwen 0.99 > Idefics2 0.93 > InternVL3 0.89 > LLaVA 0.7-0.8) 따라감 — H-encoder-saturation 의 second downstream signature. LLaVA-Next M5a positive (LM-side 10/10) + M5b NULL → physics-mode commitment 가 LLaVA family 에서 LM 통해 routing, encoder 아님. Insight: `docs/insights/m5b_sae_intervention_cross_model.md`. (b) **§4.6 Idefics2 L26-31 deeper-layer disambiguation**: fresh M2 LM 캡처 at L26/28/30/31 (Mistral-7B 32 layer, 81-97 % depth) + v_L extraction (n_pos=470, n_neg=10) + 80-run counterfactual sweep × n=10 × 2 config (~65 분). **결과: 0/40 v_unit + 0/40 random**, 단 1 isolated noise hit at L28. v_L projection 모든 depth 에서 깨끗하게 ascending. **Wrong-relative-depth hypothesis falsified across L5-L31 (16-97 % depth)**; **perceiver-resampler 가 leading remaining mechanism candidate** (5-모델 design 이 projector 단독 격리 못 함; controlled projector-swap test out of scope until M-PSwap). H-shortcut framing: pixel-encodability 는 architecture-conditional with projector design (MLP vs perceiver) 이 disambiguating axis. Insight: `docs/insights/sec4_6_cross_model_revised.md`. | (이 commit) |
| 2026-04-28 (audit) | **Track-B 시기 추가 사항 종합 review audit (Phase 1+2+3 + post-projection SAE + M-PSwap feasibility + cross-model M5b)** — 결과는 minor correction 으로 audit survive: ✅ Phase 2 의 H2 cell 15개 모두 post-Phase-3 lexicon expansion 후에도 양수 유지 (max +0.006 label-averaged, +0.033 single-cell); ✅ Idefics2 NULL on M5a + M5b describe 가 robust (under-perturbation 아님; 30/30 framing shift 단일 cell + random 10/10 kinetic 보존 — single-cell scope, 2nd-cell test 필요); ⚠️ **cross-method M5a/M5b "agreement" 비대칭** (1 positive/positive cell + 3 null/null, 4 independent positives 아님) — doc 언어 tighten; ⚠️ **Generative-vs-Categorical claim 이 Qwen 특이적, architecture-level 아님**; ⚠️ design 이 2 generative + 1 categorical prompt — Qwen 내부에서도 "categorical task" vs "yes/no binary format" 분리 못 함 → MCQ probe (audit follow-up #1) 가 load-bearing; ⚠️ Phase 3 cell 선택이 constraint-driven (M5a baseline-low vs M5b baseline-high); ⚠️ roadmap 언어 "5-model behavioral coverage + 2-model causal coverage" 로 tighten. 철회된 claim 없음; 일부 qualified. Insight: `docs/insights/review_audit_2026-04-28.md`. | `fda84e8` |
| 2026-04-28 (Phase 3 lexicon tightening) | **Audit follow-up: "30/30" headline tighten + Phase 3 lexicon-monotonicity note** — Idefics2 single-cell framing-shift "30/30 stim" 재계산 narrows to "10 stim × 3 k value × 1 unique intervention text"; "30 independent stim" → "1 cell × 10 stim × 3 k 반복" framing. Phase 3 `score_describe` lexicon 확장 (`impact`, `about to (fall|...)`, `in motion`) 이 Phase 2 re-score 에서 non-monotone-safe 검증 (max delta +0.033 single cell). Doc 언어 lock. | `5341f67` |
| 2026-04-28 (저녁 M-MP audit follow-up) | **M-MP audit follow-up MCQ probe + Idefics2 2nd-cell test ✅** — audit follow-up #1 (multi-choice categorization, "categorical task" vs "yes/no binary format" 분리) + #1b (Idefics2 2nd cell, single-cell framing-shift architecture-level 로 lift 시도) 종결. **(a) MCQ probe**: 신규 `meta_phys_mcq` prompt (A real-world physical event / B geometric figure / C symbol/icon / D none) + `score_meta_phys_mcq` scorer in `pmr.py`. Phase 1 smoke (5 모델 × n=48 = 720 추론, 100% parse rate gate 5 모델 모두 통과). Phase 2 full (5 모델 × 1440 stim × 1 prompt = 7200 추론, GPU 1 ~75분). Phase 3 Qwen MCQ M5a + M5b at audit-pinned cell: M5a α=40 → 0/10 flip (yesno NULL 와 매칭, describe 10/10 와 NOT); M5b top_k=20 → 10/10 break (describe break 와 매칭, yesno NO break 와 NOT). **Qwen × MCQ 의 cross-method split**: M5a method null (yesno 와 매칭) 그러나 M5b method break (describe 와 매칭). 통일된 "Generative-vs-Categorical" framing 이 split — yes/no 의 M5b immunity 는 **yes/no-prompt-specific** 으로 기술 (n=1 categorical-binary prompt). Behavioral H2 cross-prompt-conserved status: 15/15 → **19/20 (model × prompt) cell 양수** — Qwen × MCQ 가 lone exception (Δ = −0.050, label-image-mismatch 상호작용). **(b) Idefics2 2nd-cell test** (`textured/ground/cast_shadow ball`): baseline 이 이미 suspended ("The ball is in the air."), kinetic 아님 — top-k SAE ablation no-op (specificity 확인: SAE feature 가 kinetic-verb production 인코드). 그러나 cell-1 framing-shift 를 architecture-level 로 lift 못 함 (3rd cell with kinetic baseline 필요). 신규: `META_PHYS_MCQ_TEMPLATE` in `prompts.py`, `score_meta_phys_mcq` in `pmr.py`. Cell-1 audit caveat: Option A 의 verbosity-preference confound; 2x2 (task × format) 의 "generative + yes/no" cell 결손. Insight: `docs/insights/m_mp_phase3_followup_2026-04-28.md`. | `2b13a53` (+ 이전 commit) |
| 2026-04-28 / 2026-04-29 (M-PSwap Pillar B WIP) | **Pillar B projector-swap LoRA infra 구축; training NaN-blocked** — feasibility spike (`38302ec` / `10bafd3`): bypass-only 실험 FAIL (perceiver-resampler 가 forward pass 에 integral, 단순 제거 불가); full LoRA training 필수. Infra commit (`d35d512` / `69634e7`): 신규 `src/physical_mode/lora/` 패키지 with `idefics2_mlp_resampler.py` (MLPPoolResampler: 1-layer cross-attn pool + 2-layer GELU MLP, fp32 master weight 로 bf16 round-off 회피) + `load_swapped.py`; training pipeline `scripts/m_pswap_{train,smoke,regression_eval,post_training}.py` (LoRA rank-32 alpha-64 on Mistral q/v/k/o_proj + MLP-pool full-finetune, `HuggingFaceM4/the_cauldron` 5 subset × 2K = 10K sample streaming, NaN-abort + per-step g_lora vs g_pr logging, resume-from-checkpoint); 진단 `m_pswap_{diagnose_nan,diagnose_nan_v2,repro_nan_batch,discriminator}.py`. **Smoke (50 step) PASS**. **Full training step 1000 에서 NaN** (run `outputs/mpswap_run_20260429-033238/step1000`); 진단 suite 가 long-text sample / streaming the_cauldron / bf16-vs-fp32 attention-pool / mask-negation pattern stress-test — WIP. NaN 해결 전까지 B1 §4.6/M5b 재실행 차단. Fallback (paper_gaps.md): B1 drop, B2 + 문헌 기반 이론 주장 유지. | `38302ec`, `10bafd3`, `d35d512`, `69634e7` |
| 2026-04-29 (housekeeping) | **Review pptx untrack + 모든 `docs/**/*_ko.md` 를 `_local/` 로 이동** — bilingual rule 이 2026-04-27 에 `docs/` 에서 retired; 50개 KO mirror 모두 workspace `_local/` 로 frozen archive. `references/{roadmap,project}_ko.md` 는 의도적으로 유지. `*.pptx` review deck 이 git tracking 에서 제외 + `.gitignore` 추가 (deck 자체는 로컬에 유지). | `0d10f2c`, `53fc062` |
