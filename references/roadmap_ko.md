...
| **H-direction-specificity** (§4.6-derived) | v_L10 방향 픽셀-공간 gradient ascent 가 PMR 을 뒤집고 매칭 magnitude random 방향은 못함 — regime flip 은 magnitude 가 아니라 방향 특이성 필요. | **Qwen 에서 지지; §4.6 cross-model 후 행동-수준 Qwen-only** | §4.6 (Qwen, 2026-04-26): ε=0.05 에서 5/5 v_L10 flip vs 0/15 random (매칭 ε=0.1, 비교 가능 projection magnitude ~80 vs ~100). **§4.6 cross-model on LLaVA-1.5 (2026-04-26)**: projection-수준 특이성은 *보존* (v_L10 이 150-200 도달 vs random 2-10), 그러나 **행동-수준 특이성은 무너짐** (둘 다 0/5 flip). "v_L10 이 행동을 뒤집는 축" 이라는 읽기는 Qwen-특이적. |
| **H-shortcut** (§4.6-derived; 2026-04-26 수정) | Shortcut 해석은 이미지 자체에 인코드 가능 — 픽셀 기반, runtime hidden-state injection 만의 속성이 아님. | **Qwen-scoped (encoder-saturation 특이적)** | §4.6 (Qwen): ε=0.05 perturbation 으로 픽셀-공간 gradient ascent 가 PMR flip. **§4.6 cross-model (2026-04-26)**: (a) Transfer test 4 비-Qwen 모델에서 0/140 flip (큰 ε 에서 InternVL3 음의 transfer); (b) LLaVA-1.5 per-model gradient ascent — projection 8→150-200 (Qwen 매칭) 상승하지만 0/5 PMR flip. 픽셀-인코드 가능 shortcut 경로는 encoder-saturation 특이적 — Qwen 의 saturated SigLIP 이 LM 이 직접 읽는 얇은 픽셀-to-L10 채널 만듦; LLaVA-1.5 의 unsaturated CLIP 은 그렇지 않음. M9 PMR-천장 / §4.7 결정-안정성 천장과 평행: 각각 같은 architecture-level saturation 의 별도 시그니처. |
