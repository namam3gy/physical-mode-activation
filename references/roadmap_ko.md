...
| **H-direction-specificity** (§4.6-derived; **2026-04-26 저녁 수정**) | v_L 방향 픽셀-공간 gradient ascent 가 PMR 을 뒤집고 매칭 magnitude random 방향은 못함 — regime flip 은 magnitude 가 아니라 방향 특이성 필요. | **supported, model-conditional layer** | §4.6 (Qwen, 2026-04-26 오전): L10 에서 ε=0.05 에서 5/5 v_L10 flip vs 0/15 random. **§4.6 cross-model layer sweep on LLaVA-1.5 (overnight followup)**: L25 가 ε=0.2 에서 5/5 v_L25 flip, ε=0.1 에서 4/5; 모든 테스트 layer 에서 random 0/15. 이전 "behavior-level Qwen-only" reading 은 L10 wrong-layer-choice artifact — LLaVA-1.5 의 shortcut layer 는 L25 (78% 깊이), Qwen L10 (36%) 와 다름. |
| **H-shortcut** (§4.6-derived; **2026-04-26 저녁 재수정**) | Shortcut 해석은 이미지 자체에 인코드 가능 — 픽셀 기반. | **supported, model-conditional layer (Qwen 한정 아님)** | §4.6 (Qwen): L10 에서 ε=0.05 perturbation 으로 픽셀-공간 gradient ascent 가 PMR flip. **§4.6 cross-model proper (overnight followup, 2026-04-26 저녁)**: LLaVA-1.5 layer sweep (L5/L10/L15/L20/L25) 결과 **L25 가 ε=0.2 에서 5/5 v_L flip, ε=0.1 에서 4/5; random control 모든 layer 0/15**. 이전 "Qwen-scoped" reading 은 wrong-layer-choice artifact — L10 (Qwen) 은 LM 36% 깊이; LLaVA-1.5 의 L25 는 78% 깊이. 픽셀-인코드 가능성은 5 architecture 중 적어도 2 개에서 일반화, 각 모델이 자기 자신의 shortcut layer 가짐. LLaVA-1.5 L25 ε=0.2 응답 샘플: "The circle will be hit by a ball." Random control: "covered by the moon." 깊이 분석: `docs/insights/sec4_6_cross_model_revised.md` (+ ko). |
...
| **§4.6 cross-model** | **픽셀-인코드 가능성 cross-model 테스트** | (a) Transfer test: Qwen-derived synth stim → 4 비-Qwen 모델 0/140 flip; InternVL3 음의 transfer (1.0→0.2 unconstrained). (b) LLaVA-1.5 per-model gradient ascent: projection 8→150-200 상승 (Qwen magnitude 매칭) 그러나 **모든 ε 에서 0/5 PMR flip**. **H-shortcut Qwen-scoped 로 수정**; 픽셀-인코드 가능성은 encoder-saturation 특이적, M9 PMR-천장 / §4.7 결정-안정성 천장과 평행. | ✅ | 2026-04-26 |
