...
| **H-direction-specificity** (§4.6-derived) | Pixel-space gradient ascent along `v_L10` flips PMR; matched-magnitude random directions do not — the regime flip requires directional specificity, not perturbation magnitude. | **supported on Qwen; behavior-level Qwen-only after §4.6 cross-model** | §4.6 (Qwen, 2026-04-26): 5/5 v_L10 flips at ε=0.05 vs 0/15 random (matched ε=0.1, comparable projection magnitudes ~80 vs ~100). **§4.6 cross-model on LLaVA-1.5 (2026-04-26)**: projection-level specificity *preserved* (v_L10 reaches 150-200 vs random 2-10), but **behavior-level specificity collapses** (both 0/5 flips). The "v_L10 is the axis that flips behavior" reading is Qwen-specific. |
| **H-shortcut** (§4.6-derived; revised 2026-04-26) | Shortcut interpretation is encodable in the image itself — pixel-driven, not just at runtime hidden-state injection. | **Qwen-scoped (encoder-saturation specific)** | §4.6 (Qwen): pixel-space gradient ascent flips PMR via ε=0.05 perturbation. **§4.6 cross-model (2026-04-26)**: (a) Transfer test 0/140 flips on 4 non-Qwen models (InternVL3 negative transfer at unconstrained); (b) LLaVA-1.5 per-model gradient ascent — projection rises 8→150-200 (matches Qwen) but 0/5 PMR flip. Pixel-encodable shortcut path is encoder-saturation specific — Qwen's saturated SigLIP creates a thin pixel-to-L10 channel the LM reads from; LLaVA-1.5's unsaturated CLIP doesn't. Parallel to M9's PMR-ceiling and §4.7's decision-stability ceiling: each is a separate signature of the same architecture-level saturation. |
