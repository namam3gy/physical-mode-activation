...
| **H4** (pilot-derived) | The open vs forced-choice PMR gap is a stable signature of the **language-prior ↔ visual-evidence** conflict. | **supported (Qwen-only — cross-model FC untested)** | M2 (Qwen): gap present at every object_level (line 32 pp → textured 22 pp). Higher abstraction ⇒ larger gap. M6 r7 cross-model M2 was open-prompt only (no FC for non-Qwen due to LLaVA "A" greedy bias and uncertain Mistral behavior). Cross-model FC remains open. |
...
| **H-locus** (M4-derived; revised 2026-04-26 evening via M5b SIP; **further refined 2026-04-26 night via attention/MLP knockout**) | The bottleneck is at LM mid layers (L10 specifically), not earlier or in the decoding head. | **supported and twice-refined — L9 MLP constructs the physics-mode commitment, L10 reads it out; attention is single-layer redundant** | M5a: L10 α=40 flips 10/10 abstract → physical responses; later layers do not move. M5a-ext Exp 3: L10 regime-flip holds across cells. M5b SIP patching (sufficiency, 2026-04-26 evening): patching corrupted's L0-L9 with clean's full visual-token hidden state recovers physics-mode in 20/20 pairs (IE=+1.0); L10-L11 60% recovery; L14+ zero. **M5b attention/MLP knockout (necessity, 2026-04-26 night, this round)**: zeroing **L9 MLP** output flips **20/20 clean stim physics → abstract (IE_necessity=+1.0)** — most localized causal finding in project. Attention knockout 0/20 break at every L0-L27 (single-layer attention is fully redundant; residual stream + surviving MLPs reconstitute it). Partial MLP necessity ring: L8 +0.4, L10 +0.6, L11 +0.4, L14 +0.4. Triangulation: L9 is **both sufficient (SIP) AND necessary (knockout)** — L9's MLP *constructs* physics-mode representation, L10 *reads* it out; off-by-one M5a/M5b reconciliation = two views of same decision boundary. H10 ("2-3 narrow IE bands") supported with revision: 1 dominant MLP band at L9 + 4 partial echoes; attention has no IE bands. |
...
| **H-direction-specificity** (§4.6-derived; **revised 2026-04-26 evening**) | Pixel-space gradient ascent along `v_L` flips PMR; matched-magnitude random directions do not — the regime flip requires directional specificity, not perturbation magnitude. | **supported, model-conditional layer** | §4.6 (Qwen, 2026-04-26 morning): 5/5 v_L10 flips at ε=0.05 vs 0/15 random. **§4.6 cross-model layer sweep on LLaVA-1.5 (overnight followup, 2026-04-26 evening)**: L25 admits 5/5 v_L25 flips at ε=0.2 and 4/5 at ε=0.1; random 0/15 at every tested layer. The prior "behavior-level Qwen-only" reading was a wrong-layer-choice artifact at L10 — LLaVA-1.5's shortcut layer is L25 (78% depth) vs Qwen's L10 (36% depth). |
| **H-shortcut** (§4.6-derived; **revised again 2026-04-26 evening**) | Shortcut interpretation is encodable in the image itself — pixel-driven. | **supported, model-conditional layer (NOT Qwen-only)** | §4.6 (Qwen): pixel-space gradient ascent flips PMR via ε=0.05 perturbation at L10. **§4.6 cross-model proper (overnight followup 2026-04-26 evening)**: LLaVA-1.5 layer sweep (L5/L10/L15/L20/L25) reveals **L25 admits 5/5 v_L flips at ε=0.2 and 4/5 at ε=0.1; random controls 0/15**. The prior "Qwen-scoped" reading was a wrong-layer-choice artifact — L10 (Qwen) is at 36% LM depth; L25 in LLaVA-1.5 is 78% depth. Pixel-encodability generalizes across at least 2/5 architectures, with each model having its own shortcut layer. Sample LLaVA-1.5 L25 ε=0.2 response: "The circle will be hit by a ball." Random control: "covered by the moon." Insight: `docs/insights/sec4_6_cross_model_revised.md` (+ ko). |
...
| **§4.6 cross-model** | **Pixel-encodability cross-model test** | (a) Transfer test: Qwen-derived synth stim → 0/140 flips on 4 non-Qwen models; InternVL3 shows negative transfer (1.0→0.2 at unconstrained). (b) LLaVA-1.5 per-model gradient ascent: projection rises 8→150-200 (matches Qwen magnitude) but **0/5 PMR flip at every ε**. **H-shortcut revised → Qwen-scoped**; pixel-encodability is encoder-saturation-specific, paralleling M9's PMR-ceiling and §4.7's decision-stability ceiling. | ✅ | 2026-04-26 |
...
| M5b | ST4 Phase 3 — SIP + patching + knockout + SAE | (1) Semantic Image Pairs + activation patching (visual-token full-state) on Qwen2.5-VL: 20 SIP pairs × 28 LM layers — sharp L10 boundary, IE=+1.0 at L0-L9, +0.6 at L10-L11, 0 at L14+. (2) Cross-model SIP+patching on LLaVA-1.5 (n=15 × 32 layers): lock-in starts at L20 (62.5% relative depth) vs Qwen L10 (36%) — curve shape replicates, locus shifts in relative depth. Idefics2 / InternVL3 too saturated for SIP construction. (3) **Attention + MLP knockout (necessity, 2026-04-26 night)**: 20 clean stim × 28 layers × 2 ablations. Attention knockout IE=0 at every layer (single-layer attention fully redundant). MLP knockout: **L9 IE=+1.0 (uniquely necessary, 0/20 retain physics)**, partial necessity ring L8 +0.4 / L10 +0.6 / L11 +0.4 / L14 +0.4. Triangulation: L9 MLP *constructs* commitment (necessary); L0-L9 representations *carry* it (sufficient via SIP); L10 *reads* it (M5a steering). Per-head attention knockout + SAE intervention still open. | partial ✅ (layer-level patching + knockout done; per-head attn + SAE remain) | 2026-04-26 |
