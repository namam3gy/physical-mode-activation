...
| **§4.6 cross-model** | **Pixel-encodability cross-model layer sweep (5 architectures × n=10) + Idefics2 9-layer disambiguation + InternVL3 alt-baseline** | 6 rounds: 2026-04-26 morning (Qwen+transfer test) → 2026-04-26 overnight (LLaVA-1.5 layer sweep) → 2026-04-27 afternoon (LLaVA-Next + Qwen layer sweep) → 2026-04-27 night 5-model n=10 chain (Idefics2 + InternVL3 added) → **2026-04-28 Idefics2 L26-31 deeper-layer disambiguation** (new M2 capture at L26/28/30/31, fresh v_L extraction, 80-run sweep) → **2026-05-03 InternVL3 alt-baseline sweep** (M8a `square_filled_ground_none_fall` / `open_no_label` / square-only v_L extracted from existing M8a captures; n=5 baselines × 5 layers × 2 configs = 50 runs in ~13 min on H200): **L10 strong shortcut 5/5 PMR=0→1**, L15 partial 2/5, L20 partial 3/5, L25 null, all random controls 0/5 (specificity confirmed). Synthesized responses semantically rich ("block on inclined plane with force..."). Final picture: Qwen broad (5 shortcut layers, all 80 % +), LLaVA-Next at L20+L25 (10/10), LLaVA-1.5 at L25 only (40 % at n=10), **Idefics2 anomaly resolved** (0 clean shortcuts across 9 layers L5-L31 = 16-97 % depth → wrong-relative-depth falsified, perceiver-resampler bottleneck remaining), **InternVL3 testable under modified protocol** (canonical "circle" prompt was saturating; M8a square + open_no_label gives 0.0 baseline → L10 5/5 flip). H-shortcut → **architecture × baseline-saturation conditional**, with projector design (MLP vs perceiver) as the disambiguating axis for genuinely-null Idefics2. Insight: `docs/insights/sec4_6_internvl3_alt_baseline.md`. | ✅ | 2026-05-03 |
...
| **M-PSwap** (Pillar B) | **Controlled projector-swap LoRA on Idefics2** | Track B Pillar B / G3 fix. Replace Idefics2 perceiver-resampler with MLP projector (LoRA rank-32, encoder + LM held fixed). Re-run §4.6 + M5b. **Predicted**: if perceiver-resampler is causal, Idefics2-MLP flips on §4.6 like LLaVA-Next does. **Status (2026-04-29)**: feasibility spike done — perceiver-bypass FAILS (`38302ec` / `10bafd3`), full LoRA training required. LoRA infra built (`d35d512` / `69634e7`): `src/physical_mode/lora/{idefics2_mlp_resampler.py, load_swapped.py}` + `scripts/m_pswap_{train,smoke,regression_eval,post_training,diagnose_nan,repro_nan_batch,discriminator}.py`. Smoke (50 step) PASS. Full training NaN at step 1000 (run `outputs/mpswap_run_20260429-033238/step1000`) unresolved across multiple diagnostic sessions. **Re-prioritized 2026-04-29 → backlogged (priority lowered, not dropped)**: M-LMSwap (Pillar B B2) promoted in its place; resume opportunistically if M-LMSwap closes early. Fallback: B2 only + literature-grounded theoretical claim. | **backlogged (NaN unresolved)** | (resume opportunistically) |
| **M-LMSwap** (Pillar B) | **Controlled LM-only-swap LoRA (CLIP+Vicuna vs CLIP+Mistral)** | Track B Pillar B / G3 fix. Original premise: pair CLIP-ViT-L with Vicuna-7B and Mistral-7B holding encoder fixed → A↔B Δ-PMR isolates LM family identity. **Status (2026-05-03): DEPRIORITIZED — recipe doesn't reproduce LLaVA-1.5 saturation regime in either direction.** Variant A regression eval gate FAIL (PMR 0.869, missing-EOS-label bug → verbose runaway). Variant B regression eval gate FAIL (PMR 0.996, mode collapse to "The ball will bounce" 4-word output across 449/480 stim — LM ignores image, recipe-level over-fit). 4 bugs found across both variants (3 Mistral-tokenizer-pipeline + 1 Vicuna-template-`</s>`-omission); even after fixes neither variant lands in target regime. LM-vs-encoder dissociation claim carried by §4.6 + M5b cross-model results instead. Insight: `docs/insights/m_lmswap_recipe_failure.md`. Artifacts (ckpts, scripts, eval logs) preserved on disk for opportunistic future recipe redesign. | **dropped (recipe failure)** | n/a |
...
- **M-LMSwap** (Pillar B, **dropped 2026-05-03 — recipe failure in both variants**). Variant A (Vicuna) trained to step21000 but verbose-runaway from missing-EOS-label bug (gate 2 FAIL PMR=0.869). Variant B (Mistral) trained to step21000 with all 3 Mistral-pipeline bugs fixed (BOS / left-pad / EOS-as-PAD) but mode-collapsed to "The ball will bounce" 4-word output regardless of image (gate 1+2+3 all FAIL, only 8 unique responses across 480 stim). Recipe doesn't reproduce LLaVA-1.5 saturation regime in either direction — recipe-level redesign needed (smaller LoRA rank/α, frozen layers, different data) with no first-try-success guarantee. LM-vs-encoder dissociation claim moves to §4.6 + M5b cross-model coverage (LLaVA family M5a positive / M5b NULL — already in paper). Insight: `docs/insights/m_lmswap_recipe_failure.md`. Artifacts preserved on disk for opportunistic revival.
- **M-PSwap** (Pillar B, **backlogged** — priority lowered 2026-04-29, not dropped). Projector-swap LoRA on Idefics2 (perceiver → MLP). NaN at step 1000 unresolved after diagnostic iterations; infra (`src/physical_mode/lora/`, `scripts/m_pswap_*.py`) preserved for opportunistic resume. Fallback if not resumed: B2 (M-LMSwap) + literature-grounded theoretical claim per `paper_gaps.md` G3.
...
| — | **M5b post-projection SAE** (Qwen) | ✅ 2026-04-28: k=20 break at same threshold as pre-projection — merger preserves physics-mode commitment localization. | between weeks 2–3 | mechanism resolution (encoder pre/post-projector) |
| — | **M5b post-projection SAE round 2** (5 models × 4 cells, regime-cross capacity ladder) | ✅ 2026-05-01. Decomposes round-1 "NULL" headline into 3 phenomena (genuine NULL / baseline-already-abstract / binary-PMR-conceals). New hypothesis row H-regime-cross. Insight: `docs/insights/m5b_post_projection_cross_model.md`. **C-section follow-ups closed 2026-05-01 evening**: B3 n=30 + C1 n=30 (5 models, Wilson CI [0.00, 0.114] vs [0.886, 1.00]) + B9 ball mirror + B13 shaded mirror + B5.2 Qwen 32B parity (10 cells, 4 scale-axis findings: k threshold +20, cue sensitivity +1, ball+cast_shadow becomes projector-localizable, redundancy ladder). Insights: `m5b_post_proj_b9_b13_c1.md`, `m5b_qwen_32b_post_proj.md`. | 4 | mechanism cross-model + paper-headline reframe + within-architecture scale axis |
| B | **M-LMSwap dropped (recipe failure in both variants)** | ✅ 2026-05-03 closed-out. Variant A: gate FAIL PMR=0.869 (missing-EOS-label bug, verbose runaway). Variant B: gate FAIL PMR=0.996 (mode collapse to "The ball will bounce" 4-word output across 449/480 stim, LM ignores image). Recipe-level redesign needed with no guarantee — bad ROI vs M7. LM-vs-encoder claim moves to §4.6 + M5b cross-model. Insight: `docs/insights/m_lmswap_recipe_failure.md`. | 4 | G3 (M-LMSwap closed; M5b cross-model carries claim) |
| B | **M-PSwap** Projector-swap LoRA on Idefics2 (perceiver → MLP) | **backlogged (2026-04-29)** — priority lowered (not dropped). Infra preserved (`d35d512` / `69634e7`); diagnostic suite WIP; NaN at step 1000 unresolved after multiple sessions. Resume after M-LMSwap if time allows; otherwise lean on B2 + literature for G3. Earlier-than-planned application of `submission_plan.md` §6 Pillar-B drop rule (week 8 → week 4). | (resume opportunistically) | G3 (n=1 perceiver) |
| B | **M-LMSwap dropped 2026-05-03 — recipe doesn't reproduce LLaVA-1.5 in either direction** | A: missing-EOS-label bug → verbose runaway (PMR 0.869). B: post-3-fix mode collapse to "The ball will bounce" 4-word output (PMR 0.996, only 8 unique responses across 480 stim). Recipe-level redesign needed; bad ROI. LM-vs-encoder claim carried by §4.6 + M5b cross-model (LLaVA M5a+/M5b NULL). Insight: `docs/insights/m_lmswap_recipe_failure.md`. | n/a | G3 (closed) |
...
- If end of week 8 has not produced clean Pillar B results → drop M-PSwap, keep M-LMSwap, lean on M-Add6 + literature. **(Resolved 2026-05-03: M-LMSwap also dropped — recipe failure in both variants. LM-vs-encoder dissociation claim now carried by §4.6 + M5b cross-model results, no further controlled-LM-swap retraining planned.)**
