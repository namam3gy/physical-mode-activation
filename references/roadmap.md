...
| **M-MP** (Pillar A) | **Multi-prompt cross-task generalization** | Track B Pillar A (`references/submission_plan.md`). 5-model M8a stim × 3 prompts (kinetic prediction / free-form description / meta-categorization). Re-run M5a steering + M5b SAE intervention. Tests whether `v_L` direction is task-agnostic physics-mode commitment vs next-state-prediction-specific. Drops counting + Michotte + identity prompts (per user 2026-04-28). External benchmark (Physion / CLEVRER) → backlog. | planned | week 1–3 |
...
|---|---|---|---|---|
| A | **M-MP** Multi-prompt cross-task generalization (3 prompts × 5 models, M5a + M5b re-run) | ▶ next up | 1–3 | G1 (single-task) |
