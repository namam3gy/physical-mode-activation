...
| B | **M-LMSwap** LM-only-swap LoRA (CLIP + Vicuna vs CLIP + Mistral) | **Variant A trained 2026-04-30 (step21000)**, regression gate split — gate 2 aggregate FAIL (0.869), gate 3 baseline-cell PASS (0.000). Per-cell discrimination preserved (line→filled→shaded ramp identical to LLaVA-1.5). **A1 decision recommended: Variant B Stage 1+2 with override → A↔B per-cell Δ-PMR comparison.** Insight: `docs/insights/lmswap_a_recipe_drift.md`. Design: `docs/m_lmswap_design.md`. | 4–5 | G3 (n=1 perceiver) |
