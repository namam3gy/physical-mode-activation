...
| **M-LMSwap** (Pillar B) | **Controlled LM-only-swap LoRA (CLIP+Vicuna vs CLIP+Mistral)** | Track B Pillar B / G3 fix. Pair CLIP-ViT-L with Vicuna-7B and Mistral-7B holding encoder fixed. Re-run M5a + M5b. **Predicted**: if encoder is the bottleneck for the LLaVA-family M5b NULL, M5b NULL persists across both LM variants (CLIP+Vicuna ≈ LLaVA-1.5; CLIP+Mistral ≈ LLaVA-Next-Mistral). Conversely, an M5a flip ladder that tracks LM family would reveal LM-side modulation independent of encoder. **Status (2026-04-29)**: promoted from week 6–7 to active (replaces M-PSwap as Pillar B's load-bearing experiment). Design note in `docs/m_lmswap_design.md`. | **active (promoted)** | week 4–5 (was 6–7) |
...
- **M-LMSwap** (Pillar B, **active**, **Variant A trained 2026-04-30**). Variant A (CLIP + Vicuna) Stage 1+2 complete to step21000; regression eval split (gate 2 FAIL aggregate PMR=0.869, gate 3 PASS line/blank/none baseline=0.000 — cell-discrimination learned at late stage). Variant B (CLIP + Mistral) gate-override decision pending: A↔B per-cell Δ-PMR comparison is the natural next read since step21000 baseline matches LLaVA-1.5 floor structurally. Insights: `docs/insights/lmswap_a_recipe_drift.md`, `docs/m_lmswap_design.md`.
...
| B | **M-LMSwap Variant A trained** (Stage 1+2 to step21000) | ✅ 2026-04-30. Regression eval split: aggregate gate FAIL (PMR=0.869) but baseline-cell gate PASS (PMR=0.000). Cell-discrimination learned. A↔B per-cell Δ-PMR comparison viable. | 4 | G3 — Variant A milestone hit |
...
| B | **M-LMSwap** LM-only-swap LoRA (CLIP + Vicuna vs CLIP + Mistral) | **Variant A trained 2026-04-30 (step21000)**, regression gate split — gate 2 aggregate FAIL (0.869), gate 3 baseline-cell PASS (0.000). Per-cell discrimination preserved (line→filled→shaded ramp identical to LLaVA-1.5). **A1 in flight (2026-05-01 22:05)** — Variant B Stage 1+2 chain launched (`scripts/chain_lmswap_b.sh`, cuda:0, ~24h wall) with gate override; A↔B per-cell Δ-PMR is the planned read. Insight: `docs/insights/lmswap_a_recipe_drift.md`. Design: `docs/m_lmswap_design.md`. | 4–5 | G3 (n=1 perceiver) |
