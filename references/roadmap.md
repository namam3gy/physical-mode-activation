...
- **M-LMSwap** (Pillar B, **active** — promoted 2026-04-29 from week 6–7 to immediate). LM-only-swap LoRA (Vicuna-7B vs Mistral-7B on CLIP+ViT-L). Design note: `docs/m_lmswap_design.md`. Re-runs M5a + M5b on the LM-controlled pair to test whether LM family modulates encoder-side mechanism availability.
...
| B | **M-LMSwap** LM-only-swap LoRA (CLIP + Vicuna vs CLIP + Mistral) | **active (promoted 2026-04-29 from week 6–7)** — design note: `docs/m_lmswap_design.md` (drafting). | 4–5 (was 6–7) | G3 (n=1 perceiver) |
