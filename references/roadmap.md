...
| **H-locus** (M4-derived; **revised 2026-04-26 evening via M5b**) | The bottleneck is at LM mid layers (L10 specifically), not earlier or in the decoding head. | **supported and refined — L10 is the decision-lock-in boundary, not a single carrier** | M5a: L10 α=40 flips 10/10 abstract → physical responses; later layers do not move. M5a-ext Exp 3: L10 regime-flip holds across cells. **M5b (2026-04-26 evening, this round)**: activation patching on 20 SIP pairs × 28 LM layers — patching corrupted's L0-L9 with clean's full visual-token hidden state recovers physics-mode in **20/20 pairs (IE=+1.0)**; L10-L11 60% recovery; L13 10%; L14+ zero. L0-L9 *all* carry sufficient cue information; L10 is where decision lock-in begins; L14+ the decision is fully baked. M5a's "single causal layer" reading was the steering-specific case (additive vector aligned only with L10's natural direction); full-state patching shows the *information* exists upstream too. Aligns with Kaduri et al. 2024 ("middle 25% layers carry cross-modal flow") and Basu et al. 2024 early-layer constraint-storage finding. |
...
| M5b | ST4 Phase 3 — SIP + patching + SAE | Semantic Image Pairs + activation patching (visual-token full-state) on Qwen2.5-VL: 20 SIP pairs (cue=both clean × cue=none corrupted) × 28 LM layers. **Result: L0-L9 patching → 100% physics recovery (IE=+1.0); L10-L11 → 60%; L12 → 30%; L14+ → 0%**. Sharp L10 boundary = decision-lock-in layer (matches M5a's L10 single-causal-layer steering finding). H10 ("2-3 narrow IE bands") refined: one contiguous L0-L9 range with full IE. Head-level + attention knockout + SAE intervention still open. | partial ✅ (visual patching done; attention knockout / SAE remain) | 2026-04-26 |
