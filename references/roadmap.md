...
| **M-MP** (Pillar A) | **Multi-prompt cross-task generalization** | Track B Pillar A (`references/submission_plan.md`). **Behavioral coverage = 5-model × 3-prompt** (Phase 1 smoke n=48 + Phase 2 full n=480 × 3 labels × 3 prompts = 4320/model = 21,600 inferences total). **Causal coverage = 2-model (Qwen + Idefics2) × 2 new prompts (`describe_scene` + `meta_phys_yesno`) on M5a + M5b** — Phase 3, not 5-model. **Behavioral headline (5-model)**: H2 paired-delta (ball−circle) positive in **all 5 models × 3 prompts = 15 cells** (range +0.006 to +0.344, range unchanged after audit lexicon expansion); saturation × prompt interaction (saturated → describe most informative; unsaturated → open most informative); 0/2160 unparseable yes/no; Phase 1↔2 ±0.018. **Causal headline (2-model, refined post-verification + audit 2026-04-28)**: (i) **Qwen-specific** generative-vs-categorical dissociation: `v_L10` + top-20 SAE features causally drive `describe_scene` (10/10 flip / 0/10 break) but NOT `meta_phys_yesno` (0/10 / no break) → in **Qwen**, mechanism is generative-prompt-conserved (open + describe) and categorical-blocked (yesno). Cannot dissociate task vs format from current data (1 categorical prompt only). (ii) **Idefics2 single-cell finding**: on `shaded/ground/both ball`, top-k SAE ablation (k=160/320/500) shifts framing kinetic→suspended ("falling down" → "in the air"), 30/30 categorical (10 stim × 3 k values, 1 unique intervention text), random control 10/10 retains kinetic — framing-shift claim is **single-cell, not architecture-level**; 2nd-cell test required to lift to architectural claim. Idefics2 describe and yesno are BOTH null/null (M5a + M5b), so Idefics2 cannot test gen-vs-cat. Cross-method M5a/M5b "agreement" rests on **1 positive/positive cell (Qwen-describe) + 3 null/null cells** (Qwen-yesno, Idefics2-describe, Idefics2-yesno) — defensible but not n=4 independent confirmations. Phases 1+2+3 all ✅. | ✅ behavioral 5-model, ✅ causal 2-model | 2026-04-28 (all phases) |
...
| A | **M-MP Phase 3** Cross-prompt M5a + M5b (Qwen + Idefics2 × 3 prompts; new prompts = `describe_scene` + `meta_phys_yesno`, existing baseline = `open`) — *causal* test | ✅ 2026-04-28; **Mixed (Qwen-specific gen-vs-cat)**: Qwen (open + describe) M5a + M5b both flip/break, Qwen yesno doesn't — generative-prompt-conserved + categorical-blocked **within Qwen**. Idefics2 describe and yesno BOTH null/null — Idefics2 cannot dissociate generative-vs-categorical from these data. **Idefics2 single-cell finding**: on shaded/ground/both ball, top-k SAE ablation shifts framing kinetic→suspended ("falling" → "in the air"); 30/30 quantified (10 stim × 3 k values, 1 unique intervention text — single-cell finding, not architecture-level; 2nd-cell test required to lift). **Audit caveats (2026-04-28)**: (a) cross-method M5a-M5b "agreement" rests on **1 positive/positive cell + 3 null/null cells**, not 4 independent positive confirmations; (b) gen-vs-cat dissociation is Qwen-specific from current data. | 3 | G1 — causal evidence |
