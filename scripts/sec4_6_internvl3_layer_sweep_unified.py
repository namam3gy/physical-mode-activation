"""§4.6 — InternVL3-8B layer sweep, single-process.

Symmetric to scripts/sec4_6_qwen_layer_sweep_unified.py for InternVL3.
...
from physical_mode.synthesis.counterfactual_internvl3 import (
    gradient_ascent_internvl3,
    reconstruct_pil_internvl3,
)
...
def _latest_steering_vectors() -> Path:
    cands = sorted(PROJECT_ROOT.glob(
        "outputs/cross_model_internvl3_capture_*/probing_steering/steering_vectors.npz"))
    if not cands:
        raise FileNotFoundError("No InternVL3 steering vectors found.")
    return cands[-1]
...
def main() -> None:
    ...
    if args.output_dir is None:
        ...
        args.output_dir = PROJECT_ROOT / f"outputs/sec4_6_internvl3_layer_sweep_unified_{ts}"
    ...
    for L in layers:
        ...
        for stim_path in baselines:
            ...
            for cfg in configs:
                ...
                recon = reconstruct_pil_internvl3(out["pixel_values_final"], processor)
