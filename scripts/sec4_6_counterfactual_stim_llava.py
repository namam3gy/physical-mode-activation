"""§4.6 cross-model — LLaVA-1.5 per-model gradient ascent driver.

Mirrors `scripts/sec4_6_counterfactual_stim.py` for LLaVA-1.5. Uses
`physical_mode.synthesis.counterfactual_llava` for the standard CLIP
preprocessing variant. Loads LLaVA-1.5's own v_L10 from
...
Same configuration sweep as Qwen §4.6: 5 baseline circle stim ×
...
def _latest_steering_vectors() -> Path:
    ...
    if not cands:
        raise FileNotFoundError(
            "No LLaVA-1.5 steering vectors. Run "
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--steering-npz", type=Path, default=None,
                   help="LLaVA-1.5 steering vectors. Default = latest.")
...
def main() -> None:
    ...
    if args.output_dir is None:
        ...
        args.output_dir = PROJECT_ROOT / f"outputs/sec4_6_counterfactual_llava_{ts}"
