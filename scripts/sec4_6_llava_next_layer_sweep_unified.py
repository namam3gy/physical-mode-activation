"""§4.6 — LLaVA-Next-Mistral minimal layer sweep, single-process (load model once).

Symmetric to scripts/sec4_6_qwen_layer_sweep_unified.py for LLaVA-Next.
Tests whether layers other than L10 (where LLaVA-1.5 fails) and L25 (where
LLaVA-1.5 succeeds) admit pixel-encodability of physics-mode commitment in
the AnyRes / Mistral-LM architecture.

Per layer: n_seeds × {bounded eps=0.1 v_unit_<L>, control random eps=0.1}.

Output: outputs/sec4_6_llava_next_layer_sweep_unified/<layer>/<config>/<sid>/

Usage:
    CUDA_VISIBLE_DEVICES=1 uv run python scripts/sec4_6_llava_next_layer_sweep_unified.py \
        --layers 5,10,15,20,25 --n-seeds 3 --eps 0.1
"""
...
def _latest_steering_vectors() -> Path:
    ...
    if not cands:
        raise FileNotFoundError("No LLaVA-Next steering vectors found.")
    return cands[-1]
...
def main() -> None:
    ...
    if args.output_dir is None:
        ...
        args.output_dir = PROJECT_ROOT / f"outputs/sec4_6_llava_next_layer_sweep_unified_{ts}"
